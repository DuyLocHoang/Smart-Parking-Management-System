...
class Camera:
    def __init__(self, model, src=0,data_file = None,location = None):
        ...
        self.thread3 = threading.Thread(target=self.screen)
        self.thread.start()
        self.thread2.start()
        self.thread3.start()
    ...
    def screen(self):
        ...
        # end = 0
        # # startx = time.time()
        # test = cv2.VideoCapture(self.src, apiPreference=cv2.CAP_FFMPEG)
        # real_fps = int(test.get(cv2.CAP_PROP_FPS))
        # while self.running_thread:
        #     if self.running:
        #
        #         ret2, frame2 = test.read()
        #         Width = frame2.shape[1]
        #         Height = frame2.shape[0]
        #         overlay = frame2.copy()
        #         frame2 = cv2.resize(frame2, (Width, Height))
        #         state = {}
        #         # print(type(self.overlaps_screen))
        #         if self.overlaps_screen != [] :
        #             for parking_area, overlap_areas, id in zip(self.parked["coordinates"], np.array(self.overlaps_screen), self.parked["id"]):
        #                 # print('aasdasd')
        #                 max_IoU_overlap = np.max(overlap_areas)
        #                 # print(max_IoU_overlap)
        #                 car_id = np.argmax(overlap_areas)
        #                 cv2.putText(frame2, str(id), (parking_area[0][0], parking_area[0][1]),
        #                             cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 3)
        #                 if max_IoU_overlap < 0.15:
        #                     cv2.fillPoly(frame2, [np.array(parking_area)], (71, 27, 92))
        #                     state[str(id)] = True
        #                 else:
        #                     state[str(id)] = False
        #             cv2.addWeighted(overlay, 0.6, frame2, 0.4, 0, frame2)
        #         time.sleep(0.01)
        #         self.img_show = frame2
        #         # print('a')
            ...
            if self.running:
                try:
                    ...
                    if ret and count % 4 == 0:
                        ...
                        xxx = int(4/(time.time()-startx))
                        startx = time.time()
                        cv2.putText(frame, str(xxx), (10, 20),
